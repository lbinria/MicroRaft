...
import io.microraft.tlavalidation.models.Entry;
...
public class AppendEntriesRequest extends RequestMessage implements TLASerializer {
    ...
    public AppendEntriesRequest(String from, String to, long term, int prevLogIndex, long prevLogTerm,
            List<Entry> entries, int commitIndex, long senderClock) {
    ...
    public AppendEntriesRequest(JsonObject jsonObject) {
        super(jsonObject.get("msource").getAsString(), jsonObject.get("mdest").getAsString(),
                MessageType.AppendEntriesRequest, jsonObject.get("mterm").getAsLong(),
                jsonObject.get("mprevLogTerm").getAsLong(), jsonObject.get("mprevLogIndex").getAsInt(),
